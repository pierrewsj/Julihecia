...
- Visual mais rosa e futurista, baseado no conceito aprovado.
- Logo superior refeita usando a imagem original limpa, sem dados da barra do celular.
- Produtos agora aceitam foto pela galeria ou câmera.
- A foto aparece no catálogo do cliente e na área administrativa.
- Sem Google Sheets: foto comprimida fica salva localmente no navegador.
- Com Google Sheets: o Apps Script envia a foto ao Google Drive e salva o link na aba Produtos.

### Atualização do Apps Script
Se você já usava a versão 1, substitua o código do Apps Script por `google-apps-script.gs`, execute `setupJulihCia()` novamente e faça uma nova implantação/atualização da implantação. A coluna `imageUrl` será criada na aba Produtos.
