(()=>{
'use strict';
const V='14.4.0', LS='julih_cliente_v14';
...
function openWhatsApp(){
  const n=normalizeWhatsApp(state.publicSettings.whatsapp||CFG.whatsapp||'');
  if(!n){ toast('WhatsApp ainda não configurado.','error'); return; }
...
function normalizeInstagram(value){
  ...
  try{
    ...
    if(/(^|\.)instagram\.com$/i.test(u.hostname)){
      username=(u.pathname.split('/').filter(Boolean)[0]||'').replace(/^@/,'');
    }
  }catch(_){}
  if(!username){
    username=raw
  ...
  return username?{url:`https://www.instagram.com/${username}/`,label:`@${username}`}:{url:'',label:''};
}
function openInstagram(){
  const profile=normalizeInstagram(state.publicSettings.instagram||CFG.instagram||'');
  if(!profile.url){ toast('Instagram ainda não configurado.','error'); return; }
  window.location.href=profile.url;
}
function productImageUrl(value){const s=String(value||'').trim();if(!s)return'';let id='';try{const u=new URL(s);if(/(^|\.)drive\.google\.com$/i.test(u.hostname)){id=u.searchParams.get('id')||((u.pathname.match(/\/d\/([^/]+)/)||[])[1]||'')}}catch(_){}if(!id){const m=s.match(/[?&]id=([^&]+)/);if(m)id=decodeURIComponent(m[1])}return id?`https://drive.google.com/thumbnail?id=${encodeURIComponent(id)}&sz=w1200`:s}
...
let state={
  ...
  publicSettings:{storeName:'Julih & Cia',whatsapp:CFG.whatsapp||'',instagram:CFG.instagram||'',deliveryFee:0,pixKey:''},
  category:'Todos',tab:'home',search:'',lastSync:null,entry:'intro'
};
try{state={...state,...JSON.parse(localStorage.getItem(LS)||'{}')}}catch{}
state.publicSettings={storeName:'Julih & Cia',whatsapp:CFG.whatsapp||'',instagram:CFG.instagram||'',deliveryFee:0,pixKey:'',...(state.publicSettings||{})};
state.entry='intro';
...
function productForOrderItem(i){return (state.products||[]).find(p=>String(p.id)===String(i?.productId||''))}
function isReadyOnlyOrder(o){const items=o?.items||[];return items.length>0&&items.every(i=>{const p=productForOrderItem(i);return !!p&&p.readyStockEnabled===true})}
function progressHtml(s,o){if(s==='cancelado')return `<div class="order-cancelled">Pedido cancelado</div>`;const ready=isReadyOnlyOrder(o)&&s!=='producao';const statuses=ready?['recebido','confirmado','pronto','entregue']:['recebido','confirmado','producao','pronto','entregue'];const labels=ready?['Recebido','Confirmado','Pronto','Entregue']:['Recebido','Confirmado','Produção','Pronto','Entregue'];let ix=statuses.indexOf(s);if(ix<0)ix=0;const n=ix+1;return `${ready?'<div class="ready-flow-note">⚡ Pedido de pronta entrega</div>':''}<div class="order-progress">${labels.map((_,i)=>`<span class="${i<n?'on':''}"></span>`).join('')}</div><div class="order-progress-labels">${labels.map((x,i)=>`<b class="${i<n?'on':''}">${x}</b>`).join('')}</div>`}
...
function productCard(p){
  ...
  return `<article class="product-card ${p.featured===true||String(p.featured)==='true'?'featured':''}"><div class="product-visual">${p.imageUrl?`<img src="${esc(productImageUrl(p.imageUrl))}" alt="${esc(p.name)}" onerror="this.remove();this.parentElement.insertAdjacentHTML('beforeend','<span>${esc(p.emoji||'🍰')}</span>')">`:`<span>${esc(p.emoji||'🍰')}</span>`}${ready?`<span class="ready-badge ${out?'out':''}">${out?'Esgotado':`${qty} disponíveis`}</span>`:''}</div><div class="product-content"><span class="badge">${esc(p.category)}</span><h3>${esc(p.name)}</h3><p>${esc(p.description||'')}</p><span class="price">${price}</span><div class="product-actions"><button class="btn ${out?'btn-soft':'btn-primary'} btn-sm" ${out?'disabled':''} data-add="${esc(p.id)}">${out?'Esgotado':Number(p.price||0)>0?'Adicionar':'Orçamento'}</button></div></div></article>`
}
function ordersView(){const arr=[...state.orders].sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));return `<main class="shell"><section class="glass orders-hero"><span class="eyebrow">Acompanhamento em tempo real</span><h1>Seus pedidos</h1><p>Veja cada etapa da sua encomenda com um visual mais moderno e elegante.</p><button class="btn btn-soft btn-sm" data-action="sync-orders">↻ Atualizar status</button></section><div class="orders-stack">${arr.length?arr.map(o=>`<article class="order-card"><div class="order-tech">Pedido conectado</div><div class="order-top"><div><span class="order-id">${esc(o.id)}</span><h3>${esc(o.items?.[0]?.name||'Pedido')}</h3></div><span class="status status-pill st-${esc(o.status)}">${statusLabel(o.status)}</span></div><div class="order-meta"><span>📅 ${fmtDate(o.deliveryDate)}</span><span>⏰ ${esc(o.deliveryTime||'A combinar')}</span><span>🧁 ${esc(o.items?.[0]?.name||'Pedido')}</span></div>${progressHtml(o.status,o)}<div class="order-line"><span>Valor</span><strong>${Number(o.total||0)>0?money(o.total):'Orçamento'}</strong></div><div class="order-line"><span>Etapa atual</span><strong>${statusLabel(o.status)}</strong></div></article>`).join(''):`<div class="card empty"><div class="emoji">📋</div><h3>Nenhum pedido ainda</h3><p>Quando você finalizar uma encomenda, ela aparecerá aqui.</p><button class="btn btn-primary" data-tab="home">Ver catálogo</button></div>`}</div></main>`}
function contact(){
  const w=normalizeWhatsApp(state.publicSettings.whatsapp||CFG.whatsapp||'');
  const ig=normalizeInstagram(state.publicSettings.instagram||CFG.instagram||'');
  return `<main class="shell contact-shell">
    <section class="contact-hero card">
      <span class="eyebrow">Contato direto</span>
      <h1>Fale com a Julih & Cia</h1>
      <p>Dúvidas, alterações e pedidos especiais com atendimento rápido.</p>
      <div class="contact-options">
        <article class="social-card">
          <div class="social-icon whatsapp-icon" aria-hidden="true">💬</div>
          <div class="social-copy"><strong>WhatsApp</strong><span>Pedidos, dúvidas e atendimento direto.</span></div>
          <button class="btn btn-primary social-button" data-action="whatsapp" ${w?'':'disabled'}>Abrir WhatsApp</button>
        </article>
        <article class="social-card">
          <div class="social-icon instagram-icon" aria-hidden="true">◎</div>
          <div class="social-copy"><strong>Instagram</strong><span>${ig.label?`Siga ${esc(ig.label)} e veja novidades.`:'Veja novidades, bolos e doces publicados.'}</span></div>
          <button class="btn btn-instagram social-button" data-action="instagram" ${ig.url?'':'disabled'}>Abrir Instagram</button>
        </article>
      </div>
    </section>
    ${state.publicSettings.pixKey?`<div class="card pix-card"><h3>💠 Pix</h3><p>Chave informada pela confeitaria:</p><strong>${esc(state.publicSettings.pixKey)}</strong></div>`:''}
  </main>`
}
...
document.addEventListener('click',e=>{const t=e.target.closest('[data-action],[data-tab],[data-category],[data-add],[data-remove]');if(!t)return;
  ...
  switch(t.dataset.action){
    ...
    case 'instagram': openInstagram(); break;
  }
});
