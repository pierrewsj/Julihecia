(() => {
    ...
    app.innerHTML = `<main class="shell">
      <section class="hero">
        <div class="hero-brand">
          <img src="assets/logo-banner.webp" alt="Logo Julih & Cia">
          <div class="hero-caption"><h1>Julih & Cia</h1><p>Feito com amor em cada detalhe.</p></div>
        </div>
        <div class="hero-actions">
          <span class="eyebrow">Pedidos + Gestão</span>
          <h2>Um aplicativo para o cliente e para a confeitaria.</h2>
          <p>Faça encomendas, acompanhe pedidos e centralize estoque, despesas, produção e financeiro em um único lugar.</p>
          <div class="entry-grid">
            <button class="entry-card" data-action="open-client"><span class="emoji">🛍️</span><h3>Quero fazer um pedido</h3><p>Catálogo, carrinho, encomendas e acompanhamento.</p><span class="btn btn-primary btn-block">Entrar</span></button>
            <button class="entry-card" data-action="open-admin-login"><span class="emoji">👩‍🍳</span><h3>Área da Confeitaria</h3><p>Pedidos, produção, estoque, financeiro e relatórios.</p><span class="btn btn-cocoa btn-block">Entrar</span></button>
          </div>
          <div class="tiny-note">💡 Esta versão funciona em modo local mesmo sem Google Sheets. A conexão pode ser ativada em <b>Configurações</b> na área administrativa.</div>
        </div>
      </section>
  ...
  function adminProducts(){
    return `<div class="page-title"><div><h1>Produtos</h1><p>Itens exibidos no catálogo do cliente.</p></div><button class="btn btn-primary" data-action="new-product">+ Novo produto</button></div><div class="product-grid">${state.products.map(p=>`<article class="product-card"><div class="product-visual">${esc(p.emoji||'🍰')}</div><div class="product-content"><span class="badge">${esc(p.category)}</span><h3>${esc(p.name)}</h3><p class="muted">${esc(p.description||'')}</p><span class="price">${Number(p.price)>0?money(p.price):'Orçamento'}</span><div class="product-actions"><button class="btn btn-soft btn-sm" data-product-edit="${esc(p.id)}">Editar</button></div></div></article>`).join('')}</div>`;
  }
  ...
  async function syncFromSheets(silent=false){
    ...
    try{
      ...
      if(Array.isArray(data.products) && data.products.length) state.products=data.products.map(p=>({...p,price:Number(p.price||0),active:String(p.active)!=='false'}));
  ...
  document.addEventListener('submit', async (e)=>{
    ...
    if(form.id==='productForm'){
      let p=fd.id?state.products.find(x=>x.id===fd.id):null;if(!p){p={id:uid('PROD')};state.products.push(p);}Object.assign(p,{name:fd.name,category:fd.category,emoji:fd.emoji||'🍰',price:Number(fd.price||0),active:fd.active==='true',description:fd.description});save();closeModal();render();toast('Produto salvo.','success');await sendMutation('upsertProduct',p);return;
    }
