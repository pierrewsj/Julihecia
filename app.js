(()=>{
'use strict';
const V='2.3.0', LS='julih_cliente_v2';
...
if(!state.entry) state.entry='intro';
...
function appbar(){return `<header class="appbar"><div class="appbar-in"><div class="brand"><img src="assets/logo.png" alt="Julih & Cia" onerror="this.style.opacity='0'"><div><strong>${esc(state.publicSettings.storeName||'Julih & Cia')}</strong><small>Pedidos & encomendas</small></div></div><button class="icon-btn" data-action="sync" aria-label="Atualizar">↻</button></div></header>`}
...
function intro(){return `<main class="intro-page"><div class="intro-art"><img src="assets/intro.png" alt="Julih & Cia" loading="eager"></div><div class="intro-overlay"></div><div class="intro-bottom"><button class="cta-main" data-action="enter-store"><span>Fazer pedido</span></button></div></main>`}
function render(){
  document.body.className = state.entry==='intro' ? 'intro-mode' : 'watermark-mode';
...
function home(){
  ...
  return `<main class="shell shell-home">
    <section class="hero-scene"><img src="assets/home-banner.png" alt="Julih & Cia — Feito com amor em cada detalhe"><div class="hero-sparkle"></div></section>
    <section class="welcome-card">
      <div class="welcome-copy">
        <span class="eyebrow">Seja bem-vinda! ♡</span>
        <h1>Delícias feitas especialmente para tornar seus momentos inesquecíveis.</h1>
        <p>Escolha seus produtos, personalize sua encomenda e receba tudo com carinho.</p>
      </div>
      <div class="welcome-action">
        <button class="cta-main cta-inside" data-action="scroll-catalog"><span>Fazer pedido</span></button>
      </div>
    </section>
    <section class="feature-grid">
      <button class="feature-card" data-category="Pronta entrega"><span class="feature-icon">🛍️</span><div><strong>Pronta entrega</strong><small>Delícias disponíveis para envio imediato.</small></div><b>›</b></button>
      <button class="feature-card" data-category="Personalizados"><span class="feature-icon">✨</span><div><strong>Personalizados</strong><small>Crie do seu jeito, com amor e sabor.</small></div><b>›</b></button>
    </section>
    <section id="catalogSection" class="catalog-wrap">
      <div class="page-title"><div><h2>Nosso catálogo</h2><p>${state.lastSync?'Atualizado '+new Date(state.lastSync).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):`Catálogo disponível • ${featuredCount()} destaques`}</p></div></div>
...
if('serviceWorker' in navigator && location.protocol.startsWith('http')) navigator.serviceWorker.register('./sw.js?v=3.0-clean').catch(()=>{});
