...
const IMAGE_FOLDER_NAME = 'Julih & Cia - Fotos dos Produtos';
...
const HEADERS = {
  Produtos: ['id','name','category','price','emoji','active','description','updatedAt','imageUrl'],
...
function doPost(e){
  try{
    ...
    switch(action){
      ...
      case 'uploadProductImage': return json_(uploadProductImage_(data));
...
function uploadProductImage_(data){
  ...
  const safeName = String(data.productName || 'produto').replace(/[^a-zA-Z0-9À-ÿ _-]/g,'').trim().replace(/\s+/g,'-').slice(0,60) || 'produto';
  const fileName = `${String(data.productId || 'PROD')}-${safeName}-${Date.now()}.${ext}`;
